...
couleurs = ["#fff","#000", "#f00", "#ff0", "#0f0", "#00f", "#f0f"]   # couleurs est un tableaux de textes de couleurs
...
def creerBoutons(couleurs, taille, espace, couleurEffacer):
    ...
    for _ in range(len(couleurs) + 1):
        if i == 0:
            effacer = True
            boutons.append(struct(coin1=coin1Tab[i], coin2=coin2Tab[i], couleur=couleurEffacer, effacer=effacer))
        else:
            effacer = False
            boutons.append(struct(coin1=coin1Tab[i], coin2=coin2Tab[i], couleur=couleurs[i - 1], effacer=effacer))
        i += 1
    return boutons
...
def dessinerBoutons(couleurs, taille, espace, couleurEffacer):
    ...
    for i in range(len(couleurs) + 1):
        if i == 0:
            fillRectangle(coin1Tab[i].x, coin1Tab[i].y, taille, taille, couleurEffacer)
        else:
            fillRectangle(coin1Tab[i].x, coin1Tab[i].y, taille, taille, couleurs[i - 1])
...
# enregistrement de coordonnées cartésiennes (position) pour déterminer si le 
# point du paramètre position se trouve dans un des carrés formés par les 
# boutons.

def trouverBouton(boutons, position):


# La procédure dessinerRectangleFlottant anime le rectangle flottant tant que 
# le bouton de la souris de l'utilisateur reste enfoncé. Elle prend comme 
# paramètre imageOriginal, un tableau de tableaux de textes de couleurs, 
# debut, un enregistrement qui contient les coordonées cartésiennes du clic 
# initial de l'utilisateur, et couleur, le text de la couleur du rectangle.
...
# tableau de tableaux de textes de couleurs, et rectangle, un enregistrement 
# qui contient deux champs (coin1 et coin2). coin1 et coin2 sont aussi des 
# enregistrements qui représentent des coordonnées cartésiennes.

def restaurerImage(imageOriginale, rectangle):


# La procédure ajouterRectangle modifie une section du paramètre image. Image 
# est un tableau de tableaux de texte de couleurs. rectangle est un 
...
# l'utilisateur. Cette procédure détermine si le clic a lieu sur un bouton de 
# couleurs, le bouton effacer, ou dans la fenêtre de dessin.
...
def testDessiner():
    ...
    assert coin1(["#fff","#000"], 12, 6) == [struct(x=6, y =6), struct(x=24, y=6), struct(x=42, y=6)]

    assert coin2([],12 , 6) == [struct(x=18, y=18)]
    assert coin2(["#fff"], 12, 6) == [struct(x=18, y=18),struct(x=36, y=18)]
    assert coin2(["#fff","#000"], 12, 6) == [struct(x=18, y =18), struct(x=36, y=18), struct(x=54, y=18)]

    assert creerBoutons([], 12, 6, "#fff") == [struct(coin1=struct(x=6, y=6), coin2=struct(x=18, y=18), couleur="#fff", effacer=True)]
    assert creerBoutons(["#fff"], 12, 6, "#fff") == [struct(coin1=struct(x=6, y=6), coin2=struct(x=18, y=18), couleur="#fff", effacer=True), struct(coin1=struct(x=24, y=6), coin2=struct(x=36, y=18), couleur="#fff", effacer = False)]
    assert creerBoutons(["#fff","#000"], 12, 6, "#fff") == [struct(coin1=struct(x=6, y=6), coin2=struct(x=18, y=18), couleur="#fff", effacer=True), struct(coin1=struct(x=24, y=6), coin2=struct(x=36, y=18), couleur="#fff", effacer=False), struct(coin1=struct(x=42, y=6), coin2=struct(x=54, y=18), couleur="#000", effacer=False)]
    assert creerBoutons(["#000","#f00"], 12, 6, "#00f") == [struct(coin1=struct(x=6, y=6), coin2=struct(x=18, y=18), couleur="#00f", effacer=True), struct(coin1=struct(x=24, y=6), coin2=struct(x=36, y=18), couleur="#000", effacer=False), struct(coin1=struct(x=42, y=6), coin2=struct(x=54, y=18), couleur="#f00", effacer=False)]
    # assert creerBoutons() ==
